...
namespace gamesmith { namespace maths {
	...
	struct mat4
	{
		...
		static mat4 perspective(float fov_degree, float aspectRatio, float near, float far)
		{
			mat4 result(1.0f);

			float q = 1.0f / tan(toRadians(0.5f * fov_degree));
		...
		static mat4 translate(const mat4& original, const vec3f& translation)
		{
		...
		static mat4 rotate(float angle_degree, const vec3f& axis)
		{
			return rotate(mat4(1.0f), angle_degree, axis);
		}
		static mat4 scale(const mat4& original, const vec3f& scale)
		{
			mat4 result = original;
		...
		{
			return mat4::scale(mat4(1.0f), scale);
		}
